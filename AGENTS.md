...
A commercial NinjaTrader 8 product line, built from scratch. Two phases:

1. **Market structure indicator** - non-repainting swing detection with HH/HL/LH/LL
   labelling, BOS lines and break markers, retest zones, and failed break / CHoCH
   marking. Sells on clarity and correctness; makes no performance claim.
2. **Automated strategy** - an NQ/MNQ scalping engine built on the same structure
   core, running the pipeline: Market Structure -> Setup -> Confirmation ->
   Qualification -> Risk Check -> Entry -> Management -> Reset. Every entry must
   have an identifiable reason. Built only if phase 1 research supports one.

Deferred: PK Trade Accounting integration, prop-account (Apex-style) protections.

> Greenfield. There is no external or inherited codebase. Concept vocabulary is
> borrowed from a public job posting for an unrelated product; no code is.
...
| Task | How |
|---|---|
| Compile | NinjaScript Editor, then Compile (F5). No CLI equivalent. |
| Backtest | Control Center, New, Strategy Analyzer |
| Optimize | Strategy Analyzer, Optimize |
